...
<form action="{{ route('save.job') }}" method="POST" >
    ...
    @if($savedJob > 0)
    <button  class="btn btn-block btn-success btn-md" disabled><span class="icon-heart-o mr-2 text-danger"></span>You saved this Job</a>
    @else
    <button name="submit" type="hidden" class="btn btn-block btn-light btn-md"><span class="icon-heart-o mr-2 text-danger"></span>Save Job</a>
...
<form action="{{ route('apply.job') }}" method="POST" >
    ...
    <input name="company" type="hidden" value="{{ $job->company }}">
    @if($appliedJob > 0)
    <button   class="btn btn-block btn-primary btn-md" disabled>Already applied</a>
    @else
    <button type="submit" name="submit" class="btn btn-block btn-primary btn-md">Apply Now</a>
    @endif
...
<li class="mb-2"> <a href="{{ route('categories.single',$category->name) }}" class="text-decoration-none"> {{ $category->name }}</a> </li>
@endforeach
</ul>
</div>
